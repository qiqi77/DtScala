...
object higher_order_functions {
  def main(args: Array[String]): Unit ={
    ...
    Array(3.14, 1.42, 2.0).map{(x: Double) => 3 * x}.foreach(println)                            //两种不同写法

    def high_order_functions(f: (Double) => Double) = f(0.25)
    println(high_order_functions(ceil _))
    println(high_order_functions(sqrt _))                                    //让函数 作用于具体数据

    def mulBy(factor: Double) = (x: Double) => factor * x
    val quintuple = mulBy(5)                                                 //此时factor=5
    println(quintuple(20))                                                   //x=20
    ...
    val fun2 = 3 * (_: Double)                                     //必须要知道占位符的类型，否则会报错
