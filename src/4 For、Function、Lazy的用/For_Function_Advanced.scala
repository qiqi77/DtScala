...
object For_Function_Advanced {
  def main(args: Array[String]): Unit ={
    ...
    val add = (x: Int) => x +200  //右侧是个匿名函数  (因为函数有值，所以可以把函数赋值给常量）函数式有值的(*)
    println("The result from a function is: " + addA(2))
    println("The result from a val is : " + add(2))

    def fac(n: Int): Int = if(n <= 0) 1 else n * fac(n - 1)  //Scala中递归调用函数 要指明返回值
    ...
    println("The result from a combine is : " + combine("I love Spark", "<<")) //传入的参数 会覆盖掉 默认值
